...
class BaseCascadeForest(BaseEstimator, metaclass=ABCMeta):
    def __init__(
        ...
        self.predictor_no_binner = predictor_no_binner
    ...
    def _scale_data(self, scaler, X, is_training_data=True):
        """
        Scale data X. If X is training data, the scaler is fitted first."""
    ...
    def _get_scaler(self, scaler_idx):
        """Get the scaler from the internal container with the given index."""
        if not 0 <= scaler_idx <= self.n_layers_:
            msg = (
                "The scaler index should be in the range [0, {}], but got {}"
                " instead."
            )
            raise ValueError(msg.format(self.n_layers_, scaler_idx))

        scaler_key = "scaler_{}".format(scaler_idx)

        return self.binners_[scaler_key]

    def _set_scaler(self, scaler_idx, scaler):
        """
        Register a scaler into the internal container with the given index."""
        scaler_key = "scaler_{}".format(scaler_idx)
        if scaler_key in self.binners_:
            msg = (
                "Scaler with the key {} already exists in the internal"
                " container."
            )
            raise RuntimeError(msg.format(scaler_key))

        self.binners_.update({scaler_key: scaler})
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if self.use_predictor:
            ...
            if self.predictor in ("forest", "xgboost", "lightgbm"):
                ...
                if self.predictor_no_binner:
                    msg = "predictor_no_binner only used for custom predictor."
            ...
            if not self.predictor_no_binner:
                binner_ = Binner(
                    n_bins=self.n_bins,
                    bin_subsample=self.bin_subsample,
                    bin_type=self.bin_type,
                ...
                X_scaled_aug_train_ = self._scale_data(
                    scaler_, snapshot_X_aug_train_, is_training_data=True
                )
                
                X_middle_train_ = _utils.init_array(X, self.n_aug_features_)
                
                X_middle_train_ = _utils.merge_array(
                    X_middle_train_, X_scaled_aug_train_, self.n_features_
                )
                self._set_scaler(self.n_layers_, scaler_)
            ...
            tic = time.time()
            if self.predictor_.__class__.__name__ == 'CTRNN':
                self.input_shape, self.output_shape = self.predictor_.fit(
                X_middle_train_, y, sample_weight, return_shape=True)
            else:
    ...
    def save(self, dirname="model"):
        ...
        d["predictor_no_binner"] = self.predictor_no_binner
        if self.predictor_.__class__.__name__ == 'CTRNN':
            d["predictor_input_shape"] = self.input_shape
            d["predictor_output_shape"] = self.output_shape
        ...
        if self.use_predictor:
            if self.predictor_.__class__.__name__ == 'CTRNN':
                _io.model_saveobj(
                    dirname, "CTRNN_predictor", self.predictor_, self.partial_mode
                )
            else:    
                _io.model_saveobj(
    ...
    def load(self, dirname, custom_predictor=None):
        """
        ...
        self.predictor_no_binner = d["predictor_no_binner"]
        ...
        if self.use_predictor:
            pred_path = os.path.join(dirname, "estimator", "predictor.h5")
            if os.path.exists(pred_path):
                self.predictor_ = _io.model_loadobj(dirname, "CTRNN_predictor",
                                                    d, custom_predictor)
            else:
...
class CascadeForestRegressor(BaseCascadeForest, RegressorMixin):
    def __init__(
        ...
        super().__init__(
            ...
            random_state=random_state,
            verbose=verbose,
            predictor_no_binner=predictor_no_binner,
        )
    ...
    def predict(self, X):
        ...
        if self.use_predictor:
            ...
            if not self.predictor_no_binner:
                ...
                scaler_ = self._get_scaler(self.n_layers_)
                X_aug_test_ = self._scale_data(
                    scaler_, X_aug_test_, is_training_data=False
                )
                X_middle_test_ = _utils.init_array(X, self.n_aug_features_)
                X_middle_test_ = _utils.merge_array(
                    X_middle_test_, X_aug_test_, self.n_features_
                )
